...
class SudokuCanvas(context: Context, private var sudoku: Sudoku) : View(context) {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (!startedRequest) {
            sudoku.SudokunuGetir()
            startedRequest = true
        }
    }
    override fun onDraw(canvas: Canvas?) {
        //BURASI BUTONA TIKLANDIKTAN SONRA GÖZÜKEN YAZININ FONKSİYONU
        super.onDraw(canvas)
        if (canvas != null && this.sudoku.hazir()) {
            canvas.drawBitmap(extraBitmap, 0f, 0f, defaultPaint)
            drawRectangles(6, 6, canvas, defaultPaint, puzzleTileLength, leftMargin, topMargin)
            drawRectangles(3, 3, canvas, boldPaint, puzzleTileLength*3, leftMargin, topMargin)
            drawNumbers(6, 6, canvas, textPaint, constantPaint, outlinePaint, puzzleTileLength, leftMargin, topMargin,
                this.sudoku.puzzle, this.sudoku.solution)
            drawKeyboard(canvas, buttonPaint, textPaint)
        } else if (canvas != null) {
            canvas.drawBitmap(extraBitmap, 0f, 0f, defaultPaint)
            canvas.drawText("Doluyor", this.leftMargin.toFloat(), this.topMargin.toFloat(), this.textPaint)
    ...
    private fun drawKeyboard(canvas: Canvas, buttonPaint: Paint, textPaint: Paint) {
        ...
        val resetText = "Resetleyin"
        val checkText = "Kontrol Edin"
        ...
        canvas.drawText(checkText,
            ...
            (topX + this.puzzleTileLength/2.0 + this.textSize/2.0).toFloat(), textPaint)
        if (this.sudoku.isOnay()) {
            this.sudoku.inDogrulama()
            var text = "Maalesef Doğru Değil"
            if (this.sudoku.cozum()) {
                text = "Doğru!"
            }
            canvas.drawText(
                text,
                    ...
                    if (event.y >= indexY && event.y <= indexY + this.keyboardKeyLength) {
                        ...
                        if (counter in 1..10) {
                            ...
                            if (event.x <= indexX + this.keyboardKeyLength) {
                                if (counter == 1)
                                    this.sudoku.sayiSil(this.currI, this.currJ)
                                else
                                    this.sudoku.sayiEkle(this.currI, this.currJ, counter - 1)
                            }
